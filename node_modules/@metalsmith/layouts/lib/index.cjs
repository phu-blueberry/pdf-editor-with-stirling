...
var toTransformer__default = /*#__PURE__*/_interopDefaultLegacy(toTransformer);
...
function getTransformer(ext) {
  ...
  const transformer = toTransformer__default["default"](ext);
  cache[ext] = transformer ? jstransformer__default["default"](transformer) : false;
  return cache[ext];
}
...
function render({
  ...
  debug.info('Rendering "%s" with layout "%s"', filename, layout); // Stringify file contents
  ...
  const layoutPath = path__default["default"].join(metalsmith.path(settings.directory), layout); // Transform the contents
...
function validate({
  ...
  debug.info(`Validating ${filename}`); // Files without a layout cannot be processed
  ...
  if (!isUtf8__default["default"](file.contents)) {
...
function layouts(options) {
  return function layouts(files, metalsmith, done) {
    ...
    debug('Running with options: %o', settings); // Check whether the pattern option is valid
